...
@ManagedBean
@ViewScoped
public class ReportTransaction extends BaseBean implements Serializable {

    private Date fromDate;
    private Date toDate;
    private String chooseCp;
    private String typeSearch;
    List<Map<String, Object>> listTrans;
    private static final Logger log = Logger.getLogger(ReportTransaction.class);
    ...
    public void onReprotTotal() {

        log.info("ReportTransaction.onReprotTotal");
        ...
        if (!CommonUtils.isNullOrEmpty(chooseCp) && !"-1".equals(chooseCp)) {
            ...
            if (isOK) {
                ...
                for (int i = 0; i < lstNewChoose.size(); i++) {
                    HashMap<String, String> hashInfo = (HashMap<String, String>) lstNewChoose.get(i);
        ...
        listTrans = dbProcessor.getTransactionProcessor().getReportTotal(fromDate, toDate, by, bankCode, listCpId);
    }
    ...
    public StreamedContent exportExcel() {
        ...
        try {
            ...
            String reportFileName = "reportTotal_" + DateTimeUtils.convertDateToString(new Date(), "yyMMddHHmmss") + ".xls";
            String reportFilePath = reportFolder + reportFileName;
            String templateFilePath = templateFolder + "reportTotal.xls";
