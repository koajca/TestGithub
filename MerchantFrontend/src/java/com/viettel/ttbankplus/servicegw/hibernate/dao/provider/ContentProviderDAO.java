...
public class ContentProviderDAO extends GenericDAO<ContentProvider, Long> {
    ...
    public List<HashMap<String, String>> getTreeProvider(Long startWith) {
        List lst;
        SQLQuery sqlQuery = getSession().createSQLQuery("select content_provider_id, cp_code, cp_name from CONTENT_PROVIDER CONNECT BY PRIOR content_provider_id = parent start with content_provider_id = :cpId");
